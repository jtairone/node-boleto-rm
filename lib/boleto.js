...
var Boleto = function (options) {
 // console.log(options) //comentar depois
  if (!options) {
    throw 'No options provided initializing Boleto.'
  }
  options['cedente_cnpj'] = options['cedente_cnpj'].replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5") 
  options['pagador_cpf_cnpj'] = options['pagador_cpf_cnpj'].replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5") 
  options['pagador_endereco_cep'] = options['pagador_endereco_cep'].replace(/(\d{5})(\d{3})/, "$1-$2") 
  options['cedente_endereco_cep'] = options['cedente_endereco_cep'].replace(/(\d{5})(\d{3})/, "$1-$2") 
...
//  this['linha_digitavel'] = this.bank.linhaDigitavel(this['barcode_data'])
  //this['linha_digitavel'] = this.bank.linhaDigitavel(this['linha_digitavel'])
}  */
...
Boleto.prototype.renderHTML = function (callback) {
  ...
  renderOptions['codigo_banco'] = this.bank.options.codigo
